...
-- MAGIC %md
-- MAGIC Below is the session variable, that you can use inside SQL Scripting.
...
-- MAGIC %md
-- MAGIC As those variables now exists as session variable, you can also re-declare them in SQL Scripting as local variables... but those are separates variables.
...
-- MAGIC %md
-- MAGIC You can force using session variable instead of local variables by using the session prefix:
...
begin
  declare v_var1 = '1';
  declare v_var2 = '2';
  values (session.v_var1, session.v_var2);
end;

-- COMMAND ----------

-- MAGIC %md
-- MAGIC If you work around with the declare or replace syntax, you'll get an error because this is not supported with local variables
...
-- MAGIC %md
-- MAGIC Even if local variables v_var1 and v_var2 are dropped at the END statement, session variables v_var1 and v_var2 still exists.
...
begin
  ...
  begin
    ...
    insert into log values (current_timestamp(), v_log);
...
-- COMMAND ----------
 ...
 begin
  declare v_sql_view  string;
  declare v_sql_alter string;
...
create or replace table HierarchyResult
( AncestorID      tinyint
, AncestorName    string
, ParentId        tinyint
, ParentName      string
...
begin
    ...
    repeat
      ...
      set v_rcnt = (select count(*) from HierarchyResult where DepthDiff = v_DepthDiff);
